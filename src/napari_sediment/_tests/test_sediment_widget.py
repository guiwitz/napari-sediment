...
def create_data():
    ...
    if os.path.exists('src/napari_sediment/data/synthetic/Synthetic1_123/capture/Synthetic1_123.hdr'):
        pass
    ...
    ref_height=20, channels=channels, white_ref_added_signal=2000, pattern_weight=10, pattern_width=10, random_seed=2)
    ...
    main_path = Path('src/napari_sediment/data/synthetic')
    os.makedirs(main_path.joinpath('Synthetic1_123/capture').as_posix(), exist_ok=True)
    os.makedirs(main_path.joinpath('Synthetic1_WR_123/capture').as_posix(), exist_ok=True)

    metadata = {'wavelength': [str(x) for x in np.linspace(300, 900, channels)]}
    save_image(
        hdr_file=main_path.joinpath('Synthetic1_123/capture/Synthetic1_123.hdr'),
        image=im_test, ext='raw', force=True, metadata=metadata)
    
    save_image(
        hdr_file=main_path.joinpath('Synthetic1_123/capture/DARKREF_Synthetic1_123.hdr'),
        image=dark_ref, ext='raw', force=True, metadata=metadata)
    
    save_image(
        hdr_file=main_path.joinpath('Synthetic1_WR_123/capture/DARKREF_Synthetic1_123.hdr'),
        image=dark_for_white_ref, ext='raw', force=True, metadata=metadata)
    save_image(
        hdr_file=main_path.joinpath('Synthetic1_WR_123/capture/WHITEREF_Synthetic1_123.hdr'),
...
def test_select_file(make_napari_viewer, capsys):
    ...
    imhdr_path = Path('src/napari_sediment/data/synthetic/Synthetic1_123/capture/Synthetic1_123.hdr')
